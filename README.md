...
実際に動作するデモを公開しています。PC・スマートフォンでアクセスすると、カメラ撮影から照合までの一連の流れを体験できます。

※Supabase無料プランのため、ごくまれにデモが一時的に利用できないことがあります。その際は以下の動作GIFをご覧ください。
...
実物のナンバーが手元になくても、下のサンプル画像を保存し、「ナンバーを撮影する」から選択すれば、その場で動作を試せます。

| <img src="docs/sample_5678.png" width="220"> | <img src="docs/sample_1234.png" width="220"> | <img src="docs/sample_1111.png" width="220"> |
|:---:|:---:|:---:|
| **5678** — 1名の顧客がヒット | **1234** — 複数（2名）の顧客がヒット | **1111** — 該当なしの表示 |
...
```mermaid
flowchart TD
    U["ユーザー<br/>(スマホ / PC ブラウザ)"]
    AUTH["Supabase Auth<br/>(ログイン認証)"]
    API["Next.js API Route<br/>(サーバー側)"]
    AI["Claude API<br/>(生成AIで画像認識)"]
    DB[("Supabase DB<br/>顧客データ / RLSで保護")]
